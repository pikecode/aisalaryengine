<template>
  <!-- Featured card: horizontal layout with enhanced content -->
  <div
    v-if="isFeatured"
  ...
  >
    <div class="flex flex-col h-full">
          ...
          <div class="flex flex-wrap items-center gap-2">
            ...
            <div class="flex items-center gap-1">
              ...
              <svg v-for="i in 5" :key="i" class="w-3.5 h-3.5" :class="i <= difficulty ? 'text-yellow-400' : 'text-gray-300'" fill="currentColor" viewBox="0 0 20 20">
                <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.8a1 1 0 00.951-.69l1.07-3.292z" />
              </svg>
            </div>
          </div>
        </div>
      ...
      <div class="flex items-center justify-between pt-4 border-t border-gray-200">
        ...
        <button class="px-6 py-2.5 rounded-lg bg-gray-900 text-white text-sm font-medium hover:bg-gray-800 transition-colors whitespace-nowrap">
          立即上车
        </button>
      </div>
    </div>
